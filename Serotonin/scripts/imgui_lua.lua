...
function ImGui.DragInt2  (label, v, speed, mn, mx, fmt) return vec_widget(label, v, 2, ImGui.DragInt,    speed, mn, mx, fmt) end
function ImGui.DragInt3  (label, v, speed, mn, mx, fmt) return vec_widget(label, v, 3, ImGui.DragInt,    speed, mn, mx, fmt) end
function ImGui.DragInt4  (label, v, speed, mn, mx, fmt) return vec_widget(label, v, 4, ImGui.DragInt,    speed, mn, mx, fmt) end
function ImGui.SliderFloat2(label, v, mn, mx, fmt) return vec_widget(label, v, 2, ImGui.SliderFloat, mn, mx, fmt) end
function ImGui.SliderFloat3(label, v, mn, mx, fmt) return vec_widget(label, v, 3, ImGui.SliderFloat, mn, mx, fmt) end
function ImGui.SliderFloat4(label, v, mn, mx, fmt) return vec_widget(label, v, 4, ImGui.SliderFloat, mn, mx, fmt) end
...
function ImGui.TableHeadersRow()
    ...
    dr_rect_filled(2, t.x0, r0_y, table_total_w(t), Style.FontHeight + Style.FramePadding.y * 2,
                   StyleColor(Col.TableHeaderBg), 0)
end
...
function ImGui.TableHeader(label)
    ...
    local hovered = ItemHoverable(click_x, y, click_w, h, id)
...
function ImGui.BeginMenuBar()
    ...
    mb._save_cursor_y = win.cursor_y
...
function ImGui.IsKeyPressed(name)
...
function ImGui.DragFloatRange2(label, vmin_cur, vmax_cur, speed, lo, hi, fmt)
    ...
    ImGui.SetNextItemWidth(each_w)
    vmax_cur = ImGui.DragFloat("##max", vmax_cur, speed or 1, vmin_cur, hi, fmt or "%.3f")
    ImGui.SameLine(nil, Style.ItemInnerSpacing.x)
    ImGui.Text(label)
    ImGui.PopID()
...
function ImGui.VSliderFloat(label, w, h, value, vmin, vmax, fmt)
    ...
    if visible then
        ...
        local grab_y = y + (1 - t) * (h - grab_h)
        local gc = active and StyleColor(Col.SliderGrabActive) or StyleColor(Col.SliderGrab)
        dr_rect_filled(2, x + 1, grab_y, w - 2, grab_h, gc, Style.GrabRounding)
        local vt = string.format(fmt or "%.2f", value)
        local vw, vh = text_size(vt)
...
function ImGui.StyleColorsLight()
    ...
    t[Col.ChildBg]               = COL(0.00, 0.00, 0.00, 0.00)
    t[Col.PopupBg]               = COL(1.00, 1.00, 1.00, 0.98)
    t[Col.Border]                = COL(0.00, 0.00, 0.00, 0.30)
    t[Col.BorderShadow]          = COL(0.00, 0.00, 0.00, 0.00)
    t[Col.FrameBg]               = COL(1.00, 1.00, 1.00, 1.00)
    t[Col.FrameBgHovered]        = COL(0.26, 0.59, 0.98, 0.40)
    t[Col.FrameBgActive]         = COL(0.26, 0.59, 0.98, 0.67)
    t[Col.TitleBg]               = COL(0.96, 0.96, 0.96, 1.00)
    t[Col.TitleBgActive]         = COL(0.82, 0.82, 0.82, 1.00)
    t[Col.TitleBgCollapsed]      = COL(1.00, 1.00, 1.00, 0.51)
    t[Col.MenuBarBg]             = COL(0.86, 0.86, 0.86, 1.00)
    t[Col.ScrollbarBg]           = COL(0.98, 0.98, 0.98, 0.53)
    t[Col.ScrollbarGrab]         = COL(0.69, 0.69, 0.69, 0.80)
    t[Col.ScrollbarGrabHovered]  = COL(0.49, 0.49, 0.49, 0.80)
    t[Col.ScrollbarGrabActive]   = COL(0.49, 0.49, 0.49, 1.00)
    t[Col.CheckMark]             = COL(0.26, 0.59, 0.98, 1.00)
    t[Col.SliderGrab]            = COL(0.26, 0.59, 0.98, 0.78)
    t[Col.SliderGrabActive]      = COL(0.46, 0.54, 0.80, 0.60)
    t[Col.Button]                = COL(0.26, 0.59, 0.98, 0.40)
    t[Col.ButtonHovered]         = COL(0.26, 0.59, 0.98, 1.00)
    t[Col.ButtonActive]          = COL(0.06, 0.53, 0.98, 1.00)
    t[Col.Header]                = COL(0.26, 0.59, 0.98, 0.31)
    t[Col.HeaderHovered]         = COL(0.26, 0.59, 0.98, 0.80)
    t[Col.HeaderActive]          = COL(0.26, 0.59, 0.98, 1.00)
    t[Col.Separator]             = COL(0.39, 0.39, 0.39, 0.62)
    t[Col.SeparatorHovered]      = COL(0.14, 0.44, 0.80, 0.78)
    t[Col.SeparatorActive]       = COL(0.14, 0.44, 0.80, 1.00)
    t[Col.ResizeGrip]            = COL(0.35, 0.35, 0.35, 0.17)
    t[Col.ResizeGripHovered]     = COL(0.26, 0.59, 0.98, 0.67)
    t[Col.ResizeGripActive]      = COL(0.26, 0.59, 0.98, 0.95)
    t[Col.Tab]                   = COL(0.76, 0.80, 0.84, 0.93)
    t[Col.TabHovered]            = COL(0.26, 0.59, 0.98, 0.80)
    t[Col.TabActive]             = COL(0.60, 0.73, 0.88, 1.00)
    t[Col.TabUnfocused]          = COL(0.92, 0.93, 0.94, 0.99)
    t[Col.TabUnfocusedActive]    = COL(0.74, 0.82, 0.91, 1.00)
    t[Col.PlotLines]             = COL(0.39, 0.39, 0.39, 1.00)
    t[Col.PlotLinesHovered]      = COL(1.00, 0.43, 0.35, 1.00)
    t[Col.PlotHistogram]         = COL(0.90, 0.70, 0.00, 1.00)
    t[Col.PlotHistogramHovered]  = COL(1.00, 0.45, 0.00, 1.00)
    t[Col.TableHeaderBg]         = COL(0.78, 0.87, 0.98, 1.00)
    t[Col.TableBorderStrong]     = COL(0.57, 0.57, 0.64, 1.00)
    t[Col.TableBorderLight]      = COL(0.68, 0.68, 0.74, 1.00)
    t[Col.TableRowBg]            = COL(0.00, 0.00, 0.00, 0.00)
    t[Col.TableRowBgAlt]         = COL(0.30, 0.30, 0.30, 0.09)
    t[Col.TextSelectedBg]        = COL(0.26, 0.59, 0.98, 0.35)
...
function ImGui.StyleColorsClassic()
    ...
    t[Col.TextDisabled]          = COL(0.60, 0.60, 0.60, 1.00)
    t[Col.WindowBg]              = COL(0.00, 0.00, 0.00, 0.85)
    t[Col.ChildBg]               = COL(0.00, 0.00, 0.00, 0.00)
    t[Col.PopupBg]               = COL(0.11, 0.11, 0.14, 0.92)
    t[Col.Border]                = COL(0.50, 0.50, 0.50, 0.50)
    t[Col.FrameBg]               = COL(0.43, 0.43, 0.43, 0.39)
    t[Col.FrameBgHovered]        = COL(0.47, 0.47, 0.69, 0.40)
    t[Col.FrameBgActive]         = COL(0.42, 0.41, 0.64, 0.69)
    t[Col.TitleBg]               = COL(0.27, 0.27, 0.54, 0.83)
    t[Col.TitleBgActive]         = COL(0.32, 0.32, 0.63, 0.87)
    t[Col.TitleBgCollapsed]      = COL(0.40, 0.40, 0.80, 0.20)
    t[Col.MenuBarBg]             = COL(0.40, 0.40, 0.55, 0.80)
    t[Col.ScrollbarBg]           = COL(0.20, 0.25, 0.30, 0.60)
    t[Col.ScrollbarGrab]         = COL(0.40, 0.40, 0.80, 0.30)
    t[Col.ScrollbarGrabHovered]  = COL(0.40, 0.40, 0.80, 0.40)
    t[Col.ScrollbarGrabActive]   = COL(0.41, 0.39, 0.80, 0.60)
    t[Col.CheckMark]             = COL(0.90, 0.90, 0.90, 0.50)
    t[Col.SliderGrab]            = COL(1.00, 1.00, 1.00, 0.30)
    t[Col.SliderGrabActive]      = COL(0.41, 0.39, 0.80, 0.60)
    t[Col.Button]                = COL(0.35, 0.40, 0.61, 0.62)
    t[Col.ButtonHovered]         = COL(0.40, 0.48, 0.71, 0.79)
    t[Col.ButtonActive]          = COL(0.46, 0.54, 0.80, 1.00)
    t[Col.Header]                = COL(0.40, 0.40, 0.90, 0.45)
    t[Col.HeaderHovered]         = COL(0.45, 0.45, 0.90, 0.80)
    t[Col.HeaderActive]          = COL(0.53, 0.53, 0.87, 0.80)
    t[Col.Separator]             = COL(0.50, 0.50, 0.50, 0.60)
    t[Col.Tab]                   = COL(0.34, 0.34, 0.68, 0.79)
    t[Col.TabHovered]            = COL(0.45, 0.45, 0.90, 0.80)
    t[Col.TabActive]             = COL(0.40, 0.40, 0.73, 0.84)
    t[Col.PlotLines]             = COL(1.00, 1.00, 1.00, 1.00)
    t[Col.PlotHistogram]         = COL(0.90, 0.70, 0.00, 1.00)
    t[Col.TableHeaderBg]         = COL(0.27, 0.27, 0.38, 1.00)
    t[Col.TableBorderStrong]     = COL(0.31, 0.31, 0.45, 1.00)
    t[Col.TableBorderLight]      = COL(0.26, 0.26, 0.28, 1.00)
    t[Col.TableRowBgAlt]         = COL(1.00, 1.00, 1.00, 0.07)
    t[Col.TextSelectedBg]        = COL(0.00, 0.00, 1.00, 0.35)
    t[Col.DragDropTarget]        = COL(1.00, 1.00, 0.00, 0.90)
...
function ImGui.ShowStyleEditor()
    if ImGui.BeginTabBar("##style_ed") then
        if ImGui.BeginTabItem("Sizes") then
            ...
            f("WindowBorderSize", 0,  3)
            f("WindowTitleHeight",16, 32)
            v("FramePadding",     0, 20)
            f("FrameRounding",    0, 12)
            f("FrameBorderSize",  0,  3)
            v("ItemSpacing",      0, 20)
            v("ItemInnerSpacing", 0, 20)
            f("IndentSpacing",    0, 32)
            f("ScrollbarSize",    8, 32)
            f("ScrollbarRounding",0, 16)
            f("GrabMinSize",      4, 32)
            f("GrabRounding",     0, 12)
            f("TabRounding",      0, 12)
