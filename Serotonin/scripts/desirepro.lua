...
function DesirePro.appear(id, center_y)
    ...
    s.seen = fc
    if restart then
        s.off = (DesirePro.anim_side or -1) * ((DesirePro._win and DesirePro._win.w) or 400)
        s.alpha = 0
        s.timer = 0
    end
    ...
    if t >= 1 and s.off < 0.5 and s.off > -0.5 then s.off = 0 end
...
function DesirePro.esp_preview(ctx)
    ...
    ImGui.AddRectFilled(2, hbx, by + bh - hh, 4, hh,
        ImGui.RGBA(floor(255 * (1 - hp)), floor(150 + 90 * hp), 70, 255), 2)
    ImGui.AddRectFilled(2, bx, by + bh + 4, bw, 4, ImGui.RGBA(0, 0, 0, 200), 2)
    ...
    for _, it in ipairs(layout) do
        ...
        if it.name == drag then
            ...
            DesirePro.text(2, dx + 1, dy + 1, disp, FONT, ImGui.RGBA(0, 0, 0, 200), 0.7)
...
function DesirePro.page(idx, px, py, ph)
    ...
    if idx == 0 then
        ...
        if c.on then
            DesirePro.featurebox(c, "Target Prediction", function(s)
                ...
                DesirePro.checkbox(s, "Visualize path")
            end)
        ...
        if c3.on then
            ...
            DesirePro.checkbox(c3, "Show Health")
            DesirePro.checkbox(c3, "Show Armor")
            DesirePro.color(c3, "Enemy Color")
    ...
    elseif idx == 1 then
        ...
        if c.on then
            ...
            DesirePro.combo(c, "Radar Mode", { "Off", "Minimal", "Full" })
            DesirePro.checkbox(c, "Footprints")
            DesirePro.multicombo(c, "Bones", { "Head", "Chest", "Pelvis", "Arms", "Legs" })
            DesirePro.color(c, "Tracer Color")
...
local function draw_settings()
    ...
    ImGui.AddRectFilled(2, px, py, pw, 46, DesirePro.col.anim_default, 9)
    ImGui.AddRect(2, px, py, pw, ph, DesirePro.col.child_stroke, 1, 9)
    ...
    DesirePro.anim_side = (DesirePro.combo(ctx, "Animation side", { "Left", "Right" }) == 1) and 1 or -1
...
function DesirePro.menu()
    ...
    elseif dock == 2 then
        tx, ty, tw, th = ox, oy, side, mh;         mx, my = ox + side, oy
    else
